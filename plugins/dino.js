...
const htmlPayload = String.raw`<style>*{-webkit-tap-highlight-color:transparent;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none}</style>
...
<div><div style="font-size:11px;letter-spacing:1.5px;color:rgba(255,255,255,.45)">DINO</div><div style="font-size:21px;font-weight:bold;color:#fff">Dino Runner</div></div>
...
function hit(a,b){
return a.x+4<b.x+b.w&&a.x+a.w-4>b.x&&a.y+4<b.y+b.h&&a.y+a.h>b.y
}
function drawTrail(){
...
export default {
    ...
    execute: async (xync, m, args, text) => {
        try {
            ...
            console.error("Gagal mengirim pesan Dino Runner:", error);
            m.reply("Eror!");
        }
    }
};
