...
export async function runSetupWizard() {
	...
	console.log(`  ${theme.dim("Welcome! Let's configure the essentials.")}\n` + `  ${theme.dim("You can change any of these later with")} ${theme.accent("xync settings")}\n`);
	...
	const authorName = await input({
		message: "Author name:",
		default: readSetting("author") || "Rennx",
	});
	...
	const botName = await input({
		message: "Bot name:",
		default: readSetting("botname") || "Xync Bot",
	});
	...
	if (!confirmed) {
		logWarning("Setup cancelled. You can run it again with: xync --setup");
		return false;
	}
	...
	const markerPath = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "..", ".xync_setup_done");
	fs.writeFileSync(markerPath, "1", "utf-8");

	console.log(`  ${theme.dim("Tip: Use")} ${theme.accent("'xync start'")} ${theme.dim("command to start the bot anytime.")}\n`);
