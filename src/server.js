...
const ADMIN_KEY = process.env.ADMIN_KEY || global.defaultAdminKey || 'xync';
const RELAY_SERVER_URL = 'https://bot.naze.biz.id';

async function setupDashboard(database, storeDB, xync) {
	...
	const checkAuth = (adminKey, botNumber, deviceId, targetClient) => {
		...
		if (xync?.user?.id && xync.user.id.includes(deviceId)) {
			validClient = xync;
	...
	socket.on('get_dashboard_data', (payload, callback) => {
		...
		try {
			...
			callback({
				...
				Settings: {
					owner: global.owner || [], author: global.author || 'Xyncdev', botname: global.botname || 'Hitori Bot',
