	...
	try {
		...
		await new Promise((resolve, reject) => {
			ff(tmpFileIn)
				...
				.addOutputOptions([
					'-vcodec', 'libwebp',
					// POTONG JADI KOTAK DULU -> BARU RESIZE 512
					'-vf', "crop=w='min(iw,ih)':h='min(iw,ih)',scale=512:512,setsar=1,fps=15, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse",
					'-loop', '0',
					'-preset', 'default',
					'-an', '-vsync', '0'
				])
		...
		await new Promise((resolve, reject) => {
			ff(tmpFileIn)
				...
				.addOutputOptions([
					'-vcodec', 'libwebp',
					// POTONG JADI KOTAK DULU -> BARU RESIZE 512
					'-vf', "crop=w='min(iw,ih)':h='min(iw,ih)',scale=512:512,setsar=1, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse",
					'-loop', '0',
					'-preset', 'default'
				])
				.toFormat('webp')
...
async function writeExif(media, data) {
	...
	try {
		...
		if (data) {
			...
			const { wra = data.pack_id ? data.pack_id : global.author ? global.author : 'xync-dev', wrb = data.packname ? data.packname : global.packname ? global.packname : 'Bot WhatsApp', wrc = data.author ? data.author : global.author ? global.author : 'Xyncdev', wrd = data.categories ? data.categories : [''], wre = data.isAvatar ? data.isAvatar : 0, ...wrf } = data;
