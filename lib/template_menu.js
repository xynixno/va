...
async function setTemplateMenu(xync, type, m, prefix, setv, db, options = {}) {
...
╰──────❍`
	...
	if (type == 1 || type == 'buttonMessage') {
		...
		let profile
		try {
			profile = await xync.profilePictureUrl(m.sender, 'image');
		} catch (e) {
			profile = fake.anonim
		}
		const menunya = `
...
╰─┬────❍
╭─┴─❍「 *BOT INFO* 」❍
├ *Nama Bot* : ${db?.set?.[options.botNumber]?.botname || 'Xync Bot'}
├ *Powered* : @${'0@s.whatsapp.net'.split('@')[0]}
├ *Owner* : @${owner[0].split('@')[0]}
├ *Mode* : ${xync.public ? 'Public' : 'Self'}
