...
const SERVER_ZIP = 'https://github.com/xynixno/va/archive/refs/heads/main.zip';
const VERSION_URL = 'https://raw.githubusercontent.com/xynixno/va/main/package.json';
...
async function updateSettings({
	...
	return new Promise((resolve, reject) => {
		try {
			...
			if (apikey) {
				content = content.replace(/(global\.APIKeys\s*=\s*\{[\s\S]*?'https:\/\/api\.xync\.biz\.id'\s*:\s*')[^']*(')/, `$1${apikey}$2`);
				if (global.APIKeys) global.APIKeys[global.APIs.xync] = apikey;
			}
			if (botname) {
